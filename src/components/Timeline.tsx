...
export function Timeline() {
  ...
  return <section className="timeline panel"><div className="timeline-header"><div><span className="eyebrow">APNG TIMELINE</span><h2>每張貼圖獨立動畫</h2></div>
    <select aria-label="選擇動畫貼圖" value={stickerId} onChange={(e) => setActiveId(e.target.value)}>{project.stickers.map((asset,index)=><option key={asset.id} value={asset.id}>貼圖 {index+1}</option>)}</select>
