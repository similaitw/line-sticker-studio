...
export function PhraseSelector() {
  ...
  const selectedIds = new Set(project.captionSlots.map((item) => item.phraseId));
  ...
  function toggle(entry: PhraseEntry) {
    ...
    if (project.captionSlots.length >= project.settings.count) return;
  ...
  function applyPreset(name: string) {
    ...
    const unique = [...new Set(pool)].slice(0, project.settings.count);
  ...
  return <section className="phrase-section panel">
    <div className="phrase-header"><div className="section-heading"><span>文</span><div><h2>常用文字詞庫</h2><p>{PHRASE_CATEGORIES.length} 類 · {PHRASE_LIBRARY.length} 組內建短句</p></div></div>
      <strong className={project.captionSlots.length === project.settings.count ? 'slot-count complete' : 'slot-count'}>{project.captionSlots.length} / {project.settings.count}</strong></div>
    ...
    <div className="phrase-options">{filtered.map((entry) => <label key={entry.id} className={selectedIds.has(entry.id) ? 'phrase-option selected' : 'phrase-option'}>
      <input type="checkbox" checked={selectedIds.has(entry.id)} disabled={!selectedIds.has(entry.id) && project.captionSlots.length >= project.settings.count} onChange={() => toggle(entry)} />
