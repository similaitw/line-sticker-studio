...
import { EXTERNAL_FONT_CANDIDATES, FONT_OPTIONS, ensureFontLoaded, fontByFamily, fontStackFor } from '../domain/fonts';
import { useProject } from '../state/ProjectContext';

interface Props { onUpload: (file: File) => void; onSlice: () => void; onSample: () => void; busy: boolean }
type GuideHit = { axis: 'x' | 'y'; index: number; distance: number };
const FONT_CATEGORIES = ['標準黑體', '圓體可愛', '明體優雅', '手寫感', '像素／遊戲', '日系補充'] as const;
export function SourceStage({ onUpload, onSlice, onSample, busy }: Props) {
  ...
  const selectedFont = fontByFamily(project.settings.fontFamily);
  const dragRef = useRef<{ axis: 'x' | 'y'; index: number } | null>(null); const [dragging, setDragging] = useState(false);
  const [hoverAxis, setHoverAxis] = useState<'x' | 'y' | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    void Promise.all([loadImage(project.sourceDataUrl), ensureFontLoaded(project.settings.fontFamily)]).then(([image]) => {
      if (active && canvasRef.current) drawSheetPreview(canvasRef.current, image, project.settings.rows * project.settings.columns, project.settings.rows, project.settings.columns, activeGuides,
        project.captionSlots, project.settings.fontSize, project.settings.fontFamily);
    });
    return () => { active = false; };
  }, [project.sourceDataUrl, project.settings.rows, project.settings.columns, project.settings.fontSize, project.settings.fontFamily, project.captionSlots, activeGuides]);
  ...
  function hitGuide(event: PointerEvent<HTMLCanvasElement>): GuideHit | null {
    const canvas = canvasRef.current; if (!canvas || !project.sourceDataUrl) return null;
    ...
    const hit = .018; let best: GuideHit | null = null;
  ...
  function handlePointerMove(event: PointerEvent<HTMLCanvasElement>) {
    if (dragRef.current) { updateGuide(event); return; }
    setHoverAxis(hitGuide(event)?.axis ?? null);
  }
  function endDrag(event: PointerEvent<HTMLCanvasElement>) {
    ...
    dragRef.current = null; setDragging(false); setHoverAxis(null);
  ...
  return <section className="stage panel">
    ...
    <div className="preview-controls">
      <label className="field range-field"><span>切割內縮 <b>{project.settings.padding}px</b></span><input type="range" min="0" max="40" value={project.settings.padding} onChange={(e) => updateSettings({ padding: Number(e.target.value) })} /></label>
      <label className="field range-field"><span>文字大小 <b>{project.settings.fontSize}px</b></span><input type="range" min="24" max="72" value={project.settings.fontSize} onChange={(e) => updateSettings({ fontSize: Number(e.target.value) })} /></label>
      <label className="field"><span>文字字型</span><select value={selectedFont.fontFamily} onChange={(e) => updateSettings({ fontFamily: e.target.value })}>
        {FONT_CATEGORIES.map((category) => <optgroup key={category} label={category}>
          {FONT_OPTIONS.filter((font) => font.category === category).map((font) => <option key={font.id} value={font.fontFamily}>{font.label}</option>)}
        </optgroup>)}
      </select></label>
      <div className="font-preview compact" style={{ fontFamily: fontStackFor(project.settings.fontFamily), fontSize: `${Math.min(30, project.settings.fontSize)}px` }}>
        <strong>收到 OK 謝謝 123</strong><small>{selectedFont.recommendedUse} · {selectedFont.license}</small>
      </div>
      <details className="font-sources compact"><summary>外部開源字型候選</summary>
        {EXTERNAL_FONT_CANDIDATES.map((font) => <a key={font.label} href={font.sourceUrl} target="_blank" rel="noreferrer">
          <strong>{font.label}</strong><span>{font.license} · {font.recommendedUse}</span>
        </a>)}
      </details>
    </div>
    <div className="sheet-frame">
      <canvas ref={canvasRef} width="1024" height="1024" hidden={!project.sourceDataUrl} className={dragging ? 'dragging-guides' : ''}
        data-guide-axis={dragRef.current?.axis ?? hoverAxis ?? undefined}
        onPointerDown={beginDrag} onPointerMove={handlePointerMove} onPointerLeave={() => { if (!dragRef.current) setHoverAxis(null); }} onPointerUp={endDrag} onPointerCancel={endDrag} />
    ...
    <div className="stage-actions">
      <label className="ghost-button file-button">載入圖檔分割<input type="file" hidden accept="image/png,image/jpeg,image/webp"
