...
describe('雙平台 MD 任務', () => {
  it.each(['chatgpt', 'gemini'] as const)('可建立並解析 %s 任務', (provider) => {
    ...
    expect(parseTaskMarkdown(markdown)).toMatchObject({ schema:'line-sticker-task/v2',provider,taskId:task.id,targetCount:8,count:9,rows:3,columns:3 });
    expect(markdown).toContain('me.jpg');
    expect(markdown).toContain('Do not include logos, brands, watermarks');
  });
  it('仍可解析 v1 任務',()=>{const project=createProject();const task=createGenerationTask(project);const markdown=buildTaskMarkdown(project,task).replace('line-sticker-task/v2','line-sticker-task/v1');expect(parseTaskMarkdown(markdown).schema).toBe('line-sticker-task/v1');});
});
