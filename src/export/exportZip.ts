...
export async function buildStickerZip(project:StickerProject):Promise<Blob>{const errors=validateProject(project).filter((item)=>item.level==='error');if(errors.length)throw new Error(errors[0].message);
  const zip=new JSZip(),spec=getSpec(project.type),cover=project.stickers[0].dataUrl;
  ...
  if(!spec.animated)project.stickers.forEach((asset)=>zip.file(asset.name,base64(asset.dataUrl),{base64:true}));
  else{for(const asset of project.stickers){const frames=project.animationSets[asset.id]??[];const width=spec.secondaryAnimation?480:spec.width,height=spec.secondaryAnimation?480:spec.height;const apng=await encodeApng(frames,width,height);
      if(project.type==='animated')zip.file(asset.name,apng);else{zip.file(asset.name,base64(asset.dataUrl),{base64:true});const folder=project.type==='popup'?'popup':'effect';zip.file(`${folder}/${asset.name}`,apng);}}
    const first=project.animationSets[project.stickers[0].id]??[];if(first.length)zip.file(spec.secondaryAnimation?`${project.type}/main.png`:'main.png',await encodeApng(first,spec.secondaryAnimation?480:spec.width,spec.secondaryAnimation?480:spec.height));}
