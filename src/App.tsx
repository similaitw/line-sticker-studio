...
export default function App() {
  ...
  const processSource = useCallback(async (dataUrl:string, fromProvider=true) => {
    ...
    const stickers=await sliceSheet(dataUrl,{count:project.settings.count,columns:project.settings.columns,padding:project.settings.padding,outputWidth:spec.width,outputHeight:spec.height,
  ...
  async function handleSample(){await run(async()=>processSource(createSampleSheet(project.settings.count,project.settings.columns),false));}
  async function handleExport(){setExporting(true);setError('');try{const errors=validateProject(project).filter((item)=>item.level==='error');if(errors.length)throw new Error(`尚有 ${errors.length} 項阻擋問題：${errors[0].message}`);downloadBlob(await buildStickerZip(project),`${project.type}-line-stickers.zip`);}catch(reason){setError(reason instanceof Error?reason.message:'匯出失敗');}finally{setExporting(false);}}
  return <div className="app-shell"><Header onExport={()=>void handleExport()} exporting={exporting}/><main><ProviderPanel/><TypeSelector/><PhraseSelector/><StyleSelector/><TaskPanel/>
...
function createSampleSheet(count:number,columns:number):string{const canvas=document.createElement('canvas');canvas.width=1024;canvas.height=1024;const ctx=canvas.getContext('2d');if(!ctx)throw new Error('瀏覽器不支援 Canvas');const rows=Math.ceil(count/columns),cw=canvas.width/columns,ch=canvas.height/rows;
