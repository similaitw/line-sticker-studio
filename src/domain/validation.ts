...
export function validateProject(project:StickerProject):ValidationIssue[]{const spec=getSpec(project.type);const issues:ValidationIssue[]=[];
  ...
  if(project.captionSlots.length!==project.settings.count)issues.push({level:'error',code:'CAPTION_COUNT',message:`文字槽位須剛好 ${project.settings.count} 個，目前 ${project.captionSlots.length} 個`});
  ...
  for(const asset of project.stickers){
  ...
  const hashes=new Map<string,string>();for(const asset of project.stickers){if(!asset.visualHash)continue;const previous=hashes.get(asset.visualHash);if(previous)issues.push({level:'warning',code:'DUPLICATE_VISUAL',message:`與 ${previous} 視覺內容高度相似，請確認表情動作有足夠變化`,assetName:asset.name});else hashes.set(asset.visualHash,asset.name);}
  if(spec.animated&&project.stickers.length){for(const asset of project.stickers){const frames=project.animationSets[asset.id]??[];const duration=frames.reduce((sum,frame)=>sum+frame.delayMs,0);
